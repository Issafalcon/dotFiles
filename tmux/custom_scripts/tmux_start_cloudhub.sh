...
TMUX=$(tmux new-session -c "${PROJECTS}"/wcc-cloudhub/Waters.Cloud.Hub/ClientApp -d -s CloudHub -n ClientApp)

# Create other windows.
tmux new-window -c "${PROJECTS}"/wcc-cloudhub/Waters.Cloud.Hub -t CloudHub:2 -n Api
tmux new-window -c "${PROJECTS}"/wcc-cloudhub -t CloudHub:3 -n Terminals
...
tmux splitw -c "${PROJECTS}"/wcc-cloudhub -t CloudHub:3
...
while true; do
  case "$1" in
    -d|--dev-proxy)
      tmux new-window -c "${PROJECTS}"/wcc-cloudhub -t CloudHub:4 -n Dev-Proxy
